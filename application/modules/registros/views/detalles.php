...
<form cform class="uniForm" action="" method="post" enctype="multipart/form-data" name="formCUM" id="formCUM">
  ...
  <div class="pieBtn">
		<button id="btn_scouter" type="button"><span class="ui-icon ui-icon-transferthick-e-w"></span> Cambiar Scouter</button>
  </div>
...
$(function() {
	$('#btn_scouter').click(function(){
		window.location = '<?php echo ruta('registros/cambiar_scouter/'.$this->registro->scouter_cum); ?>';
	});
	
	$('#btn_asignar').click(function(){
		window.location = '<?php echo ruta('registros/paso3/'.$this->registro->scouter_cum); ?>';
	});
	
	$('#btn_modificar').click(function(){
		window.location = '<?php echo ruta('registros/modificar/'.$this->registro->scouter_cum); ?>';
	});
